...
public class VolcanologistServiceImpl implements VolcanologistService {
    ...
    private final String VOLCANOLOGISTS_PATH = "src/main/resources/files/xml/volcanologists.xml";
    ...
    @Autowired
    VolcanologistServiceImpl(VolcanologistRepository volcanologistRepository, VolcanoRepository volcanoRepository, XmlParser xmlParser, ModelMapper modelMapper, ValidationUtil validationUtil) {
    ...
    public String importVolcanologists() throws IOException, JAXBException {
        ...
        VolcanologistImportRootDto volcanologistImportRootDto = this.xmlParser
                .fromFile(VOLCANOLOGISTS_PATH, VolcanologistImportRootDto.class);
        for (VolcanologistImportDto volcanologistImportDto : volcanologistImportRootDto.getVolcanologistImportDtos()) {
            if (this.volcanologistRepository.findByFirstName(volcanologistImportDto.getFirstName()).isPresent() ||
            this.volcanologistRepository.findByLastName(volcanologistImportDto.getLastName()).isPresent() ||
